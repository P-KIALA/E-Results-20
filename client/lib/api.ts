export async function authFetch(input: RequestInfo, init: RequestInit = {}) {
  ...
  const viteBase =
    ...
    null;
  const runtimeBase =
    (typeof window !== "undefined" ? (window as any).__APP_BASE_URL__ : null) ||
    null;
  const origin =
    viteBase ||
    runtimeBase ||
  ...
  if (asStr && asStr.startsWith("/api")) {
    ...
    if (isEmbedded) {
      ...
      try {
        ...
        lastErr = e;
        try {
          console.warn("authFetch: relative (embedded) attempt failed", {
            url: asStr,
            err: String(e),
            online:
              typeof navigator !== "undefined" ? navigator.onLine : undefined,
      ...
      try {
        ...
        lastErr = e;
        try {
          console.warn("authFetch: relative fetch failed, trying absolute", {
            url: asStr,
            err: String(e),
            online:
              typeof navigator !== "undefined" ? navigator.onLine : undefined,
